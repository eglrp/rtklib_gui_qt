...
static int iobsu =0;            /* current rover observation data index, u: user */
static int iobsr =0;            /* current reference observation data index, r: refer */
...
* return n:	number of rover and base 
*----------------------------------------------------------------------------*/
...
*		3) rtkpos(): do rtk
*		4) update filter
*		5) output sol( output every epoch or the last one )
*from:	execpos
*to:	rtkpos()
*---------------------------------------------------------------------------*/
    ...
    while ((nobs=inputobs(obs,rtk.sol.stat,popt))>=0) {
        ...
        for (i=n=0;i<nobs;i++) { 
			/* n: num of obs after discard excluded sats; 
			   i: the of obs before exclude sats */
        ...
        if (mode==0) { /* forward/backward */
			...
			/* In output single sol mode, update the sol when the these case happens: */
            else if (time.time==0||pri[rtk.sol.stat]<=pri[sol.stat]) { 
				/* update rover sol */
				sol=rtk.sol; 

				/* update base sol : THE BASE SOL WILL ONLY BE COMPUTED IN MOVING BASE MODE */
...
/* post-processing positioning: open session --------------------------------------
