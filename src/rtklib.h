...
typedef struct {        /* time struct : expand the integer type 'time' to 'float' type */
...
typedef struct {        /* solution status type: for one sat */
    gtime_t time;       /* time (GPST) */
    unsigned char sat;  /* satellite number */
...
typedef struct {        /* processing options type */
    ...
    double prn[6];      /* process-noise std [0]bias,[1]iono [2]trop [3]acch,acc:acceleration,h:horizonal [4]accv [5] pos */
